...
;General
  ...
  ;Name and file
  Name "FreeMi UPnP Media Server $3"
...
Function .onInit
  ...
  GetDLLVersionLocal "./bin/Release/FreeMi UPnP Media Server.exe" $R0 $R1
  IntOp $R2 $R0 >> 16
...
SectionIn RO
  ...
  SetOutPath "$INSTDIR"
  SetOverwrite on  	
  File /x *.pdb /x *.vshost.* /x *.manifest ".\bin\Release\*.*" 
  ...
  nsisFirewall::AddAuthorizedApplication "$INSTDIR\FreeMi UPnP Media Server.exe" "FreeMi UPnP Media Server"
...
Section "Uninstall"
  ...
  nsisFirewall::RemoveAuthorizedApplication "$INSTDIR\FreeMi UPnP Media Server.exe"
