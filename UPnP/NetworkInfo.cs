...
namespace OpenSource.UPnP
{
    ...
    {
        public static bool IPv6Enabled { get; set; }
        ...
        public NetworkInfo(InterfaceHandler onNewInterfaceSink)
        {
            ...
            if (!Utils.IsMono() && !Utils.IsWindows2000())
            {
                ...
                foreach (NetworkInterface i in interfaces)
                {
                    if (i.IsReceiveOnly == false && i.OperationalStatus == OperationalStatus.Up && i.SupportsMulticast == true)
                    {
                        ...
                        {
                            if (!AddressTable.Contains(i3.Address) && (IPv6Enabled || i3.Address.AddressFamily != AddressFamily.InterNetworkV6)
                                && !i3.Address.Equals(IPAddress.IPv6Loopback)) { AddressTable.Add(i3.Address); }
                        }
                    }
                }
            }
        ...
        private void PollInterface(LifeTimeMonitor sender, Object obj)
        {
            try
            {
                ArrayList CurrentAddressTable;
                if (!Utils.IsMono() && !Utils.IsWindows2000())
                {
                    CurrentAddressTable = new ArrayList();
                    NetworkInterface[] interfaces = NetworkInterface.GetAllNetworkInterfaces();
                    foreach (NetworkInterface i in interfaces)
                    {
                        if (i.IsReceiveOnly == false && i.OperationalStatus == OperationalStatus.Up && i.SupportsMulticast == true)
                        {
                            ...
                            foreach (UnicastIPAddressInformation i3 in i2.UnicastAddresses)
                            {
                                if (!CurrentAddressTable.Contains(i3.Address) && (IPv6Enabled || i3.Address.AddressFamily != AddressFamily.InterNetworkV6)
                                    && !i3.Address.Equals(IPAddress.IPv6Loopback)) { CurrentAddressTable.Add(i3.Address); }
                            }
                        }
                    }
                }
                ...
                {
                    IPHostEntry HostInfo = Dns.GetHostEntry(HostName);
                    CurrentAddressTable = new ArrayList(HostInfo.AddressList);
                }
