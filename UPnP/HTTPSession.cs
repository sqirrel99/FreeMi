...
namespace OpenSource.UPnP
{
    ...
    public sealed class HTTPSession
    {
        ...
        public HTTPMessage Headers = null;
        ...
        public HTTPSession(IPEndPoint Local, IPEndPoint Remote, SessionHandler CreateCallback, SessionHandler CreateFailedCallback, Object State)
        {
            ...
            MainSocket = new AsyncSocket(BUFFER_SIZE);
        ...
        public HTTPSession(Socket TheSocket, ReceiveHeaderHandler HeaderCallback, ReceiveHandler RequestCallback)
            : this((IPEndPoint)TheSocket.LocalEndPoint, TheSocket, HeaderCallback, RequestCallback)
        {
        }
        ...
        public HTTPSession(IPEndPoint Local, Socket TheSocket, ReceiveHeaderHandler HeaderCallback, ReceiveHandler RequestCallback)
            : this(Local, TheSocket)
        {
        ...
        public void SendStreamObject(Stream SObject, HTTPSession.Range[] Ranges, string ContentType)
        {
            ...
            if (Ranges != null)
            {
                ...
                if (infoObj.RangeList.Length == 1)
                {
                    Packet += "Content-Range: bytes " + infoObj.RangeList[0].Position.ToString() + "-" + (infoObj.RangeList[0].Position + infoObj.RangeList[0].Length - 1).ToString() +
                        "/" + (SObject.CanSeek ? SObject.Length.ToString() : (infoObj.RangeList[0].Position + infoObj.RangeList[0].Length).ToString()) +
        ...
        /// <param name="ContentType">Content Type</param>
        public void SendStreamObject(Stream SObject, long length, string ContentType, bool chunk_enabled)
        {
            Info InfoObj = new Info();
            InfoObj.CurrentStreamObject = SObject;
                ...
                else
                {
                    ...
                    if (this.IsConnected == true)
                    {
                        ...
                        if (infoObj.RangeList != null)
                        {
                            ...
                            else if (infoObj.RangeList[infoObj.RangeIndex].RangeLeft < BUFFER_SIZE)
                            {
            ...
            if (this.FinishedHeader == false)
            {
                    ...
                    (buffer[BufferSize - 2] == 13) && (buffer[BufferSize - 1] == 10))
                {
                    ...
                    if (this.NeedToWaitToClose == true)
                    {
                        ...
                        sender.BufferReadLength = BUFFER_SIZE;
                    }
                    else
                    {
                        if (IsChunked == true)
            ...
            if (ChunkState == HTTPSession.START_CHUNK)
            {
                ...
                if ((buffer[BufferSize - 2] == 13) && (buffer[BufferSize - 1] == 10))
                {
                    ...
                    if (ChunkDataSize != 0)
                    {
                        ...
                        MainSocket.BufferBeginPointer = BufferSize;
                        if (ChunkDataSize < BUFFER_SIZE)
                        {
                            MainSocket.BufferReadLength = ChunkDataSize;
                        }
                        else
                        {
                            MainSocket.BufferReadLength = BUFFER_SIZE;
                        }
                        return;
                    }
                    else
